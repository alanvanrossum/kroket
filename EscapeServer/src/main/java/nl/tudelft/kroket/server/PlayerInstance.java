    ...
    if (player == null) {
      ...
      try {
        ...
        if (!EscapeServer.ready()) {
          player.sendMessage(EscapeServer.countPlayers() + " player(s) connected.");
        }
  ...
  private void processInput(String input) {
    ...
    if (input.startsWith("TYPE[")) {
      ...
      if (pos > 9) {
        ...
        Player player = EscapeServer.registerPlayer(clientSocket, outputStream, name);
        ...
        LOG.error(className, "Closing bracket not found or in invalid position.");
      }
