...
public class EscapeServer implements Runnable {
  ...
  public static void main(String[] args) {
    ...
    if (initialized) {
      ...
      while (!breakLoop) {
        ...
        LOG.info(CLASSNAME, "Game not ready. Waiting for players to register...");
  ...
  public static void removePlayer(final Socket clientSocket) {
    ...
    if (player != null) {
      LOG.info(CLASSNAME, String.format("Player %s disconnected.", player.getName()));
      sendAll(String.format("Player %s has left the game.", player.getName()));
    }
   ...
   *            - the player's name
   */
  public static void registerPlayer(final Socket clientSocket, final DataOutputStream dOutput,
    ...
    if (playername.length() > 0) {
      ...
      if (getPlayer(clientSocket) == null) {
        ...
        if (getPlayer(playername) == null) {
          ...
          final Player player = new Player(clientSocket, dOutput, playername);
          ...
          player.sendMessage("You are now registered as " + player.toString());
  ...
  public void run() {
    ...
    if (serverSocket == null) {
      ...
      while (!isStopped()) {
        ...
        try {
          LOG.info(CLASSNAME, String.format("Listening for incoming connections on port %d...",
        		   serverPort));
